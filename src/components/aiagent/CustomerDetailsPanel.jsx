  ...
  return (
    <div className="flex rounded-sm overflow-hidden bg-white min-w-0 w-full max-w-[400px] shrink-0 shadow-sm">
      ...
      <div className="flex flex-col gap-2 p-2 border-r border-gray-200 shrink-0 bg-white">
        <button
          type="button"
          className={`w-10 h-10 rounded-sm ${viewMode === 'info' ? 'bg-green-600' : 'bg-white border border-gray-300'} text-white flex items-center justify-center hover:bg-green-700 transition-colors`}
          title="Information"
          onClick={() => setViewMode('info')}
        >
          <Info className={`w-5 h-5 ${viewMode === 'info' ? 'text-white' : 'text-gray-700'}`} />
        </button>
        <button
          type="button"
          className={`w-10 h-10 rounded-sm ${viewMode === 'orders' ? 'bg-green-600' : 'bg-white border border-gray-300'} flex items-center justify-center transition-colors`}
          title="Order History"
          onClick={() => setViewMode('orders')}
        >
          <RefreshCw className={`w-4 h-4 ${viewMode === 'orders' ? 'text-white' : 'text-gray-700'}`} />
        </button>
        <button
          type="button"
          className={`w-10 h-10 rounded-sm ${viewMode === 'ai' ? 'bg-green-600' : 'bg-white border border-gray-300'} flex items-center justify-center transition-colors`}
          title="AI Knowledge Base"
          onClick={() => setViewMode('ai')}
        >
          <List className={`w-4 h-4 ${viewMode === 'ai' ? 'text-white' : 'text-gray-700'}`} />
        </button>
      </div>
