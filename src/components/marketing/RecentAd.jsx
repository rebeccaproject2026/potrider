...
import { adBarChartOptions, getAdBarChartSeries } from "../../config/marketingChartConfig";
      ...
      <div className="grid grid-cols-[30%_70%] gap-2">
        ...
        <div className="mb-2">
          <select
            ...
            value={selectedAd.id}
            onChange={(e) => {
              const ad = RECENT_ADS.find((a) => a.id === parseInt(e.target.value));
              setSelectedAd(ad);
            }}
          >
            {RECENT_ADS.map((ad) => (
        ...
        <div className="text-sm flex flex-col gap-4">
          <div className="flex items-center">
            <span className="text-black font-medium text-[15px]">Impressions :</span>
            <span className="text-[#464646] font-medium">{selectedAd.impressions}</span>
          </div>

          <div className="flex items-center">
            <span className="text-black font-medium text-[15px]">Started at :</span>
            <span className="text-[#464646] font-medium">{selectedAd.startedAt}</span>
          </div>

          <div className="flex items-center">
            <span className="text-black font-medium text-[15px]">Ended at :</span>
            <span className="text-[#464646] font-medium">{selectedAd.endedAt}</span>
          </div>
        </div>
        </div>
