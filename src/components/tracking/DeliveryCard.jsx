...
const DeliveryCard = ({
  ...
  const getStatusStyle = (s) => {
    switch (s) {
      case "Delivered": return { text: "text-[#109F22]", bar: "bg-[#109F22]", width: "w-full" };
      case "Cancelled": return { text: "text-[#F44336]", bar: "bg-[#F44336]", width: "w-full" };
      case "In-progress": return { text: "text-[#FF9800]", bar: "bg-[#FF9800]", width: "w-2/3" };
      default: return { text: "text-[#0066FF]", bar: "bg-[#0066FF]", width: "w-1/3" };
    }
  };
      ...
      <div className="p-3">
          ...
          <div className="flex gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${isOnline ? "bg-[#D4FFDA] text-[#109F22]" : "bg-[#FEECEB] text-[#F44336]"
                  }`}>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between gap-2 mb-1">
            <p className="text-[11px] text-gray-500 font-medium truncate">ETA: {eta}</p>
          ...
          <div className="relative h-1 bg-gray-100 rounded-full overflow-hidden">
            <div className={`absolute left-0 top-0 h-full rounded-full ${style.bar} ${style.width}`}></div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex h-3.5 w-full rounded overflow-hidden mb-2">
            {pending > 0 && <div style={{ width: `${pPending}%` }} className="bg-[#0066FF] flex items-center justify-center text-[9px] text-white font-bold">{pending}</div>}
            {inProgress > 0 && <div style={{ width: `${pInProgress}%` }} className="bg-[#FF9800] flex items-center justify-center text-[9px] text-white font-bold">{inProgress}</div>}
            {delivered > 0 && <div style={{ width: `${pDelivered}%` }} className="bg-[#109F22] flex items-center justify-center text-[9px] text-white font-bold">{delivered}</div>}
            {cancelled > 0 && <div style={{ width: `${pCancelled}%` }} className="bg-[#F44336] flex items-center justify-center text-[9px] text-white font-bold">{cancelled}</div>}
          </div>
